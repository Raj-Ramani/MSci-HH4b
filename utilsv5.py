...
def get_data(file_path_data_18,region,half="",mc=False):
...
#     preCut="( ntag >= 4 )&(X_wt_tag>1.5)" 
#     with uproot.open("nominal_3.root:{}".format(region)) as root_file:
#         file_path_data_18 = pd.DataFrame(root_file.arrays(root_file.keys(), "(ntag != 3) & (X_wt_tag>=1.5)", library='np'))
...
#         data17['NN_weights'] = norm_17 * data17['NN_d24_weight_bstrap_med_17']
    ...
    if mc==True:
        mc_sf_np = np.atleast_2d(
                    np.stack(data18["mc_sf"]).T)
        nominal_mc_sf = mc_sf_np[0]
        data18['weight'] = 57.70 * nominal_mc_sf
