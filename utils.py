...
def get_data(file_path_data_16,file_path_data_17,file_path_data_18,region,half="",mc=False):
...
#     # specific region
    ...
    for arr in [data16, data17, data18]:
        bkt_vec = to_categorical(arr['trig_bucket'], num_classes=2)
        arr.insert(0, 'bkt_0', bkt_vec[:,0], False)
        arr.insert(0, 'bkt_1', bkt_vec[:,1], False)
...
#         else:
#             norm_16_VR = get_mu(f_data16,16,vr=True)
#             norm_17_VR = get_mu(f_data17,17,vr=True)
#             norm_18_VR = get_mu(f_data18,18,vr=True)
#             data16['NN_weights'] = norm_16_VR * data16['NN_d24_weight_VRderiv_bstrap_med_16']
#             data18['NN_weights'] = norm_18_VR * data17['NN_d24_weight_VRderiv_bstrap_med_17']
...
def get_data_mask(data16,data17,data18,mask='2bRW'):
    ...
    if mask=='2bRW':
        df = data_all.loc[(data_all["ntag"] == 2)].reset_index(drop=True) 
